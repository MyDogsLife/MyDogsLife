...
public class MemberDataAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (Integer.parseInt(datas.get(position).GetCategory()) == BEAUTY)
            img_flag.setImageResource(R.drawable.haircut);
        else if (Integer.parseInt(datas.get(position).GetCategory()) == VACINE)
            img_flag.setImageResource(R.drawable.vacine);
        else if (Integer.parseInt(datas.get(position).GetCategory()) == HOSPITAL)
